...
def usage(): Unit = {
  println("Sireum Transpilers /build")
...
def compile(): Unit = {
  if (!didCompile) {
    ...
    tipe()
    println("Compiling ...")
    mill.call(ISZ("transpilers.c.jvm.tests.compile")).at(home).console.runCheck()
    println()
  }
}
...
def test(): Unit = {
  ...
  mill.call(ISZ("transpilers.c.jvm.tests")).at(home).console.runCheck()
  println()
}
...
def testJs(): Unit = {
  ...
  mill.call(ISZ("transpilers.c.js.tests")).at(home).console.runCheck()
  println()
}
...
def jitpack(): Unit = {
  ...
  val r = mill.call(ISZ("jitPack", "--owner", "sireum", "--repo", "transpilers", "--lib", "c")).
    at(home).console.run()
  r match {
...
def m2(): Unit = {
  ...
  val m2s: ISZ[ISZ[String]] =
    for (pkg <- ISZ("common", "c"); plat <- ISZ("shared", "js"))
